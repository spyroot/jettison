...
import (
	...
	_ "github.com/vmware/govmomi/vmdk"
	"log"
)
...
func CreateVmifneeded(ctx context.Context, c *vim25.Client) (string, error) {
	...
	_, _, vm, err := vcenter.VmFromCluster(ctx, c, TestImageName, TestImageCluster)
	switch t := err.(type) {
	case *vcenter.VmNotFound:
...
func UploadImage(ctx context.Context, c *vim25.Client) error {
	...
	ds, err := vcenter.FindDatastore(ctx, c, TestImageCluster, TestDataStoreName)
	if err != nil {
		return err
	}
	...
	return Import(ctx, c, TestImage, ds, ImportParams(p))
}
